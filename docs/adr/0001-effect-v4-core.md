...
- `bun run typecheck` patches the local TypeScript with `@effect/language-service` first, so it fails on a floating Effect. Not on install: the pi package install omits dev dependencies, the patcher among them.
